...
const Footer = () => {
  return (
    <footer className="bg-gray-200 flex justify-center items-center py-20 shadow-2xl">
      <div className="flex w-full flex-col gap-14 px-4 sm:px-8">
        <div className="flex flex-col items-start justify-center gap-[10%] md:flex-row">
          <Link to="/" className="font-bold text-lg mb-10">Shoppee</Link>
          ...
          <div className="flex flex-col gap-5">
            <FooterColumn title={FOOTER_CONTACT_INFO.title}>
              {FOOTER_CONTACT_INFO.links.map((link, index) => (
                <Link key={index} className="text-gray-600 flex gap-4 md:flex-col lg:flex-row">
                  <p>{link.label}:</p>
                  <p>{link.value}</p>
                </Link>
              ))}
        ...
        <div className="border-t-2 border-gray-300 pt-8">
          <p className="text-center">2024 Shoppee | All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
