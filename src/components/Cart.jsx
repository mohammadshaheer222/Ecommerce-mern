  ...
  return (
    ...
    >
      ...
      {getTotalCartAmount() === 0 ? (
        <div className="flex justify-center items-center h-full text-5xl">Cart is Empty</div>
      ) : (
        <div className="space-y-6">
          {all_products.map((itemDetails) => {
            if (cartItems[itemDetails.id] > 0) {
              return (
                <div key={itemDetails.id} className="flex flex-col py-4">
                  <div className="flex flex-col justify-center gap-y-2 items-center md:flex-row md:gap-y-0  md:gap-x-4  w-full h-full ">
                    <div>
                      <img src={itemDetails.image} alt="" width={130} />
                    </div>
                    <div>
                      <h1 className="font-semibold">{itemDetails.name}</h1>
                      <div className="flex justify-between mt-4">
                        <div className="flex cursor-pointer gap-x-2">
                          <div className="flex ring-1 ring-gray-400 text-lg px-2">
                            <p onClick={() => decrement(itemDetails.id)}>-</p>{" "}
                            <span className="mx-2 cursor-none text-lg font-medium">
                              {getSingleCartItem(itemDetails.id)}
                            </span>{" "}
                            <p onClick={() => increment(itemDetails.id)}>+</p>
                          </div>
                          <button
                            onClick={() => removeCart(itemDetails.id)}
          ...
          <button className="btn-dark py-2 w-full">
            Checkout : $ <span className="font-semibold text-xl">{getTotalCartAmount()}.00</span>
          </button>
        </div>
      )}
    </div>
